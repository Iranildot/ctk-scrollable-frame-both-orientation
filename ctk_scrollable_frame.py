...
# Anchor map → relative position (x, y) inside the canvas (0.0 to 1.0)
_ANCHOR_POSITION = {
    "nw":     (0.0, 0.0),
    "n":      (0.5, 0.0),
    "ne":     (1.0, 0.0),
...
class CTkScrollableFrame(tkinter.Frame, CTkAppearanceModeBaseClass, CTkScalingBaseClass):
    def __init__(self,
        ...
        self._content_anchor = content_anchor.lower()

        if self._content_anchor not in _ANCHOR_POSITION:
            raise ValueError(f"Invalid content_anchor: '{content_anchor}'. "
                             f"Use one of: {list(_ANCHOR_POSITION.keys())}")
        ...
        # Vertical scrollbar (or horizontal when orientation="horizontal")
        self._scrollbar = None
        # Horizontal scrollbar (only when orientation="both")
        self._scrollbar_x = None

        if self._orientation == "horizontal":
            self._scrollbar = CTkScrollbar(master=self._parent_frame, orientation="horizontal",
            ...
            self._scrollbar = CTkScrollbar(master=self._parent_frame, orientation="vertical",
    ...
    def _apply_content_anchor(self):
        """
        Reposition the inner frame on the canvas according to content_anchor.
        When the content is smaller than the canvas, it is aligned/centered
        according to the anchor. When it overflows, scrolling takes over and
        the frame is positioned at (0, 0).
        """
        ...
        rel_x, rel_y = _ANCHOR_POSITION[self._content_anchor]

        x = max(0, int((canvas_w - frame_w) * rel_x))
        y = max(0, int((canvas_h - frame_h) * rel_y))

        self._parent_canvas.coords(self._create_window_id, x, y)

        # The scrollregion covers at least the canvas size,
        # avoiding useless scrolling when the content is small
        sr_w = max(frame_w + x, canvas_w)
        sr_h = max(frame_h + y, canvas_h)
    ...
    def _create_grid(self):
        ...
        if self._orientation == "horizontal":
            ...
            self._parent_canvas.grid(row=1, column=0, sticky="nsew", padx=(border_spacing, 0), pady=(border_spacing, 0))
            self._scrollbar.grid(row=1, column=1, sticky="nsew", pady=(border_spacing, 0))
            self._scrollbar_x.grid(row=2, column=0, sticky="nsew", padx=(border_spacing, 0))
    ...
    def _set_dimensions(self, width=None, height=None):
        """Set desired (unscaled) width/height and apply widget scaling."""
    ...
    def configure(self, **kwargs):
        ...
        if "content_anchor" in kwargs:
            new_anchor = kwargs.pop("content_anchor").lower()
            if new_anchor not in _ANCHOR_POSITION:
                raise ValueError(f"Invalid content_anchor: '{new_anchor}'. "
                                  f"Use one of: {list(_ANCHOR_POSITION.keys())}")
            self._content_anchor = new_anchor
            self._apply_content_anchor()
        ...
        if "scrollbar_fg_color" in kwargs:
            val = kwargs.pop("scrollbar_fg_color")
            self._scrollbar.configure(fg_color=val)
            if self._scrollbar_x is not None:
                self._scrollbar_x.configure(fg_color=val)

        if "scrollbar_button_color" in kwargs:
            val = kwargs.pop("scrollbar_button_color")
            self._scrollbar.configure(button_color=val)
            if self._scrollbar_x is not None:
                self._scrollbar_x.configure(button_color=val)

        if "scrollbar_button_hover_color" in kwargs:
            val = kwargs.pop("scrollbar_button_hover_color")
            self._scrollbar.configure(button_hover_color=val)
            if self._scrollbar_x is not None:
                self._scrollbar_x.configure(button_hover_color=val)
    ...
    def cget(self, attribute_name: str):
        ...
        elif attribute_name == "content_anchor":
            return self._content_anchor
    ...
    def _fit_frame_dimensions_to_canvas(self, event):
        if self._orientation == "horizontal":
            self._parent_canvas.itemconfigure(self._create_window_id, height=event.height)
        elif self._orientation == "vertical":
            self._parent_canvas.itemconfigure(self._create_window_id, width=event.width)
        # "both": no fixed dimension
    ...
    def check_if_master_is_canvas(self, widget):
        """Return True if the widget is the canvas or a descendant of it."""
